...
# конвертирование файлов из .xls в .xlsx
...
async def parsing_url(url: str) -> None:
    ...
    for i in range(3, worksheet.max_column):
        ...
        for j in range(3, worksheet.max_row):
            ...
            # если дата пустая, то ищем дату, затем оставляем только число xx-xx-xx
            index_for_day = j - 1
            while date == None:
            ...
            if value != None:
                ...
                # убираем вначале приписку из двух-трёх символов, если она есть
                ...
                for value in list_value:
                    ...
                    # если две точки есть в последнем слове, то инициалы = преподаватель
                    if text_split[-1].count(".") == 2:
                        name_teacher.append(" ".join(text_split[-2:]))
                        text_split = text_split[:-2]
                    # иначе преподавателя нет
                    else:
            ...
            # если пары нет на эту дату, время и кабинет
            else:
                await update_schedule(
